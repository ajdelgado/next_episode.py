                ...
                try:
                    ...
                    if len_cmdline > 1:
                        if cmdline[1] == sys.argv[0]:
                            log.info(
                                "Already running process %s '%s'" % (process, cmdline)
                            )
                            sys.exit(1)
...
def GetLastShowEpisode(SHOW):
    ...
    if not FILES:
        log.error(
            "No files found in '%s/%s' getting last show episode."
            % (CONFIG["path"], SHOW)
        )
        return False
    LASTEPISODE = 0
    ...
    if LASTEPISODE == 0:
        log.info(
            "There is no downloaded episodes of the show '%s' or "
            "it was the '%s'" % (SHOW, LASTEPISODE)
        )
    else:
        log.info(
            "Last episode downloaded from show '%s' was " "'%s'" % (SHOW, LASTEPISODE)
        )
    return LASTEPISODE
...
def GetEZTVShows():
    ...
    if SHOWS_CONTENT == "" or SHOWS_CONTENT is None or SHOWS_CONTENT is False:
        log.info(
            "Nothing obtained from the base url '%s'. There "
            "is no Internet connection or the URL is broken?" % CONFIG["baseurl"]
        )
        sys.exit(1)
...
def GetEZTVShowInformation(show):
    ...
    if m is not None:
        ...
        # NextEpisode
        m = re.search(
            'Next Episode is <span style="color: #0f559d;'
            ' font-weight: bold;">Season ([0-9]*) Episode '
            "([0-9]*)</span> and airs on.",
            content,
        )
    if m is not None:
        season = m.group(1)
...
def find_next_episodes():
    ...
    parser = argparse.ArgumentParser(
        description="Find next episodes for a "
    ...
    parser.add_argument(
        "--proxy", dest="proxy", help="Proxy string to use (http://127.0.0.1:8080/)"
    )
    parser.add_argument(
        "--logfile",
        dest="logfile",
        default="{}/log/next_" "episodes_2.log".format(os.environ["HOME"]),
        help="Log file to write output",
    )
    parser.add_argument(
        "--useragent",
        dest="useragent",
        default="Mozilla/5.0 (X11; Linux i686; rv:5.0) " "Gecko/20100101 Firefox/5.0",
        help="User-agent string to use to identify",
    )
    parser.add_argument(
        "--transmissionuser",
        dest="transmissionuser",
        default="admin",
        help="User name to authenticate with Transmission",
    )
    parser.add_argument(
        "--transmissionpass",
        dest="transmissionpass",
        default="",
        help="Password to authenticate with Transmission",
    )
    parser.add_argument(
        "--transmissionserver",
        dest="transmissionserver",
        default="localhost",
        help="Transmission server to connect",
    )
    parser.add_argument(
        "--transmissionport",
        dest="transmissionport",
        default="9091",
    ...
    filehandler = RotatingFileHandler(
        CONFIG["logfile"], maxBytes=1000000, backupCount=5
    )
    ...
    for SHOW in SHOWS_DIRS:
        ...
        if CONFIG["exceptions"]:
            for EXCEPTION in CONFIG["exceptions"]:
                if SHOW == EXCEPTION:
                    log.info(
                        "Skipping show '%s' due to exception "
            ...
            if not EZTVSHOW:
                ...
                if not EZTVEpisode:
                    ...
                    if "magnet" in list(EZTVEpisode.keys()):
                        ...
                        log.error(
                            "URL for torrent not found in the episode:%s" % EZTVEpisode
                        )
