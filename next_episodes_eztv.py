                ...
                try:
                    ...
                    if len_cmdline > 1:
                        if cmdline[1] == sys.argv[0]:
                            log.info("Already running process %s '%s'"
                                     % (process, cmdline))
                            sys.exit(1)
                except psutil.AccessDenied:
                    log.info("I can't read information for process %s"
                             % process)
...
def GetLastShowEpisode(SHOW):
    ...
    for FILE in FILES:
        ...
        EPISODE = 0
        m = re.match(r".*[Ss](?P<SEASON>[0-9]{1,2})[Ee](?P<EPISODE>[0-9]{1,2}).*",
                     FILE)
        if m is not None:
            RESULT = m.groupdict()
            EPISODE = int(RESULT['SEASON'])*100+int(RESULT['EPISODE'])
        else:
            m = re.match(r".*(?P<SEASON>[0-9]{1,2})x(?P<EPISODE>[0-9]{1,2}).*",
                         FILE)
            if m is not None:
                RESULT = m.groupdict()
                EPISODE = int(RESULT['SEASON'])*100+int(RESULT['EPISODE'])
        ...
        if EPISODE > LASTEPISODE:
            LASTEPISODE = EPISODE
    if LASTEPISODE == 0:
        log.info("There is no downloaded episodes of the show '%s' or "
                 "it was the '%s'" % (SHOW, LASTEPISODE))
    else:
        log.info("Last episode downloaded from show '%s' was "
                 "'%s'" % (SHOW, LASTEPISODE))
    return LASTEPISODE
...
def GetEZTVShowInformation(show):
    ...
    # NextEpisode
        m = re.search('Next Episode is <span style="color: #0f559d;'
                      ' font-weight: bold;">Season ([0-9]*) Episode '
                      '([0-9]*)</span> and airs on.', content)
    if m is not None:
        season = m.group(1)
        episode = m.group(2)
...
def SimplifyShowName(show_name):
    new_name = show_name.lower().replace(".",
                                         " ").replace("'",
                                                      "").replace("  ",
                                                                  " ").replace("(",
                                                                               "").replace(")",
...
def FindNextEpisode(last_episode):
    if last_episode == 0:
        ...
        same_season = "S%sE%s" % (sseason.rjust(2, "0"),
                                  sepisode.rjust(2, "0"))
...
parser = argparse.ArgumentParser(description='Find next episodes for a '
                                 'given path containing TV shows in '
                                 'separate folders.')
parser.add_argument('--debug', dest='debug', default='WARNING',
                    help='Set debug level (CRITICAL, '
                    'ERROR, WARNING, INFO, DEBUG, NOTSET)')
parser.add_argument('--path', dest='path',
                    default='/home/ficheros/videos/series',
...
parser.add_argument('--logfile', dest='logfile',
                    default="{}/log/next_"
                    "episodes_2.log".format(os.environ['HOME']),
                    help='Log file to write output')
parser.add_argument('--useragent', dest='useragent',
                    default="Mozilla/5.0 (X11; Linux i686; rv:5.0) "
                    "Gecko/20100101 Firefox/5.0",
                    help='User-agent string to use to identify')
parser.add_argument('--transmissionuser', dest='transmissionuser',
                    default="admin",
...
parser.add_argument('--transmissionserver', dest='transmissionserver',
                    default="localhost",
                    help='Transmission server to connect')
parser.add_argument('--transmissionport', dest='transmissionport',
                    default="9091",
                    help='Transmission server port to connect')
parser.add_argument('--baseurl', dest='baseurl',
                    default="https://eztv.io/showlist/",
...
for SHOW in SHOWS_DIRS:
    ...
    if not SKIPTHIS:
        ...
        if not EZTVSHOW:
            ...
            log.info("FOUND in '%s' in EZTV as '%s'" % (SHOW,
                                                        EZTVSHOW['name']))
            ...
            log.info("Next episode to download is %s or %s or %s"
                     % NEXT_EPISODE)
            EZTVEpisode = GetEZTVEpisodeByFileName(EZTVSHOW, NEXT_EPISODE[0])
            if 'last_episode' in list(EZTVSHOW.keys()):
                log.info("Last episode in EZTV is '%s'"
                         % EZTVSHOW['last_episode'])
            if not EZTVEpisode:
                EZTVEpisode = GetEZTVEpisodeByFileName(EZTVSHOW,
                                                       NEXT_EPISODE[1])
            if not EZTVEpisode:
                EZTVEpisode = GetEZTVEpisodeByFileName(EZTVSHOW,
                                                       NEXT_EPISODE[2])
            if not EZTVEpisode:
                ...
                if 'magnet' in list(EZTVEpisode.keys()):
                    ...
                    log.error("URL for torrent not found in the episode:%s"
                              % EZTVEpisode)
